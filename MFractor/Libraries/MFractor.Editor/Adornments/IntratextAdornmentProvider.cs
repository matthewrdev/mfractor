...
namespace MFractor.Editor.Adornments
{
        ...
        where TAdornment : UIElement
    {
        ...
        Dictionary<SnapshotSpan, TAdornment> adornmentCache = new Dictionary<SnapshotSpan, TAdornment>();
        protected ITextSnapshot Snapshot { get; private set; }
        readonly List<SnapshotSpan> invalidatedSpans = new List<SnapshotSpan>();

        readonly Comparer comparer = new Comparer();
...
#elif VS_MAC
                    view.VisualElement.BeginInvokeOnMainThread(new Action(AsyncUpdate));
#endif
                }
            }
        }
        ...
        public virtual IEnumerable<ITagSpan<IntraTextAdornmentTag>> GetTags(NormalizedSnapshotSpanCollection spans)
        {
            ...
            if (translatedSpans is null)
            {
                yield break;
            }

            var tags = GetAdornmentTagsOnSnapshot(translatedSpans).ToList();
        ...
        IEnumerable<TagSpan<IntraTextAdornmentTag>> GetAdornmentTagsOnSnapshot(NormalizedSnapshotSpanCollection spans)
        {
            ...
            foreach (var spanDataPair in GetAdornmentData(spans).Distinct(comparer))
            {
        ...
        class Comparer : IEqualityComparer<Tuple<SnapshotSpan, PositionAffinity?, TData>>
        {
            public bool Equals(Tuple<SnapshotSpan, PositionAffinity?, TData> x, Tuple<SnapshotSpan, PositionAffinity?, TData> y)
            {
                ...
                return x == null || y == null ? false : x.Item1.Equals(y.Item1);
            }
